 ...
 *   - Aggregate by A/C Name, summing the Incl Amount column.
 ...
 *
 * Returns one entry per unique account, in the order it first appears.
 */
...
export function extractDayEndDebtors(content: string): DayEndDebtorEntry[] {
  ...
  for (const line of lines) {
    ...
    if (!totals.has(titled)) order.push(titled);
    totals.set(titled, (totals.get(titled) ?? 0) + incl);
  }

  return order.map(name => ({ accountName: name, amount: totals.get(name) ?? 0 }));
}
